...
primo_tratto = curv2_ppbezierCC1_interp(P1, a, b, param);
...
secondo_tratto = curv2_ppbezierCC1_interp(P2, a, b, param);
...
tol=0.5e-2;
unione_tratto_due = curv2_mdppbezier_join(primo_tratto, secondo_tratto, tol);
unione_tratto_due.ab = 0 : length(unione_tratto_due.deg);
...
tratto_sopra_met.deg = 1;
tratto_sopra_met.ab = [0 1];
tratto_sopra_met.cp = [4.0 8.0; 6.0 8.0];

tratto_sotto_met.deg = 1;
tratto_sotto_met.ab = [0 1];
tratto_sotto_met.cp = [3.0 1.0; 6.0 1.0];

%tratto_cent_met.deg = 1;
%tratto_cent_met.ab = [0 1];
%tratto_cent_met.cp = [6.0 1.0; 6.0 8.0];

unione_met1 = curv2_mdppbezier_join(unione_tratto_due, tratto_sopra_met, tol);
unione_met1.ab = 0 : length(unione_met1.deg);
unione_met2 = curv2_mdppbezier_join(unione_met1, tratto_sotto_met, tol);
unione_met2.ab = 0 : length(unione_met2.deg);
%unione_met3 = curv2_mdppbezier_join(unione_met2, tratto_cent_met, tol);
unione_met3 = curv2_mdppbezier_close(unione_met2);

unione_sinistra_sotto = curv2_mdppbezier_join(unione_tratto_due, tratto_sotto, tol);
unione_sinistra_sotto.ab = 0 : length(unione_sinistra_sotto.deg);
unione_sotto_destra = curv2_mdppbezier_join(unione_sinistra_sotto, riflessa, tol);
unione_sotto_destra.ab = 0 : length(unione_sotto_destra.deg);
unione_completa = curv2_mdppbezier_join(unione_sotto_destra, tratto_sopra, tol);
unione_completa.ab = 0 : length(unione_completa.deg);

punti_unione_met_comp = curv2_mdppbezier_plot(unione_met3, -150, 'b', 4);

punti_unione_completa = curv2_mdppbezier_plot(unione_completa, -150, 'b', 4);
point_fill(punti_unione_completa, 'r');
point_fill(punti_unione_met_comp, 'b');
