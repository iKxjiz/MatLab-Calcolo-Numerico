...
tol=0.5e-2;
unione_tratti = curv2_mdppbezier_join(primo_tratto, secondo_tratto, tol);
unione_tratti.ab = 0 : length(unione_tratti.deg);
...
riflessa = unione_tratti;
riflessa.cp = point_trans(unione_tratti.cp, SY);
...
unione_parziale = curv2_mdppbezier_join(unione_tratti, riflessa, tol);
unione_parziale.ab = 0 : length(unione_parziale.deg);
unione_completa = curv2_mdppbezier_close(unione_parziale);

punti_unione_completa = curv2_mdppbezier_plot(unione_completa, 150, 'b', 3);
