...
fun =@(x) runge(x); % La @ indica il puntatore alla function
a=-5;
b=5;

%definisce grado del polinomio e punti di interpolazione
n=17; % Cosa succede se i punti sono pari o dispari ? Cosa succede se è grande o piccolo ?
% se troppo grande, grossi problemi (64 oppure tipo 128 da Warnings nel terminale)
% se metto n = 256, guarda la differenza tra Lagrange (nettamente migliore, molto stabile per
% gradi alti) e Bernstein


% Tipo 1 : punti equispazioati || Tipo 2 : punti non equispaziati
tipo=2; % Il numero di condizionamento aumenta per tipo 1 e diminuisce per tipo 2
switch (tipo)
    case 1
